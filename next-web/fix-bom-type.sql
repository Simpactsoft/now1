-- Final fix for BOM RPC function type mismatches
...
RETURNS TABLE (
    ...
    unit TEXT, -- Return TEXT consistently
...
BEGIN
    ...
    WITH RECURSIVE bom_explosion AS (
        SELECT
            ...
            bi.unit::TEXT, -- Explicit cast to TEXT to match return type
            bi.scrap_factor,
            bi.is_assembly,
            bi.is_phantom,
            p.sku,
            ...
            ARRAY[p.name]::TEXT[] AS path, -- Explicit cast to match return type
        ...
        SELECT
            ...
            child.unit::TEXT, -- Explicit cast to TEXT
            child.scrap_factor,
            child.is_assembly,
            child.is_phantom,
            cp.sku,
            ...
            (parent.path || cp.name)::TEXT[] AS path, -- Explicit cast to match return type
