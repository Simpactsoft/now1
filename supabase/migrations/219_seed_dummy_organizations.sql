...
-- Migration: 219_seed_dummy_organizations.sql
-- Description: Seeds 3000 dummy organizations with English and Hebrew names.
...
DECLARE
    ...
    v_is_hebrew boolean;
BEGIN
    -- 1. Get a valid Tenant ID (Pick the first one found order by created_at)
    -- Ideally this should be the 'Demo' tenant or the user's main tenant.
    SELECT id INTO v_tenant_id FROM tenants ORDER BY created_at ASC LIMIT 1;
    
    IF v_tenant_id IS NULL THEN
        RAISE EXCEPTION 'No tenant found in tenants table. Cannot seed data.';
    END IF;
