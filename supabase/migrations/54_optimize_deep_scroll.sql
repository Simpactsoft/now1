-- Migration 54: Optimize Deep Scroll (Late Row Lookup)
-- Purpose: Improve performance for deep pagination (e.g. page 1000) using Late Row Lookup strategy.
-- Strategy:
-- 1. Create Covering Indexes for common sort/filter patterns.
-- 2. Rewrite fetch_people_crm to fetch IDs first (lightweight), then join for full data.

-- [1] Covering Indexes for "Deep Sorting"
-- Essential for avoiding full table scans when sorting by created_at or status.

-- Sort by Created At (Default)
CREATE INDEX IF NOT EXISTS idx_parties_deep_created ON parties (tenant_id, type, created_at DESC, id ASC);

-- Sort by Status
CREATE INDEX IF NOT EXISTS idx_parties_deep_status ON parties (tenant_id, type, status, created_at DESC, id ASC);

-- Sort by Name (Hybrid B-Tree + Trigram already covers search, but this aids pure sort)
CREATE INDEX IF NOT EXISTS idx_parties_deep_name ON parties (tenant_id, type, display_name ASC, id ASC);


-- [2] Refactor RPC for Late Row Lookup
...
DECLARE
    ...
    v_tags_filter text[];
BEGIN
    -- 0. Parse JSON Filters (Same Logic as Mg49)
    ...
    -- 3. Get Total Count (Must be done before limit)
    EXECUTE 'SELECT count(*) FROM parties p ' || v_where_clause INTO v_total_rows;

    -- 4. Sort Logic
    -- Note: Role Sort still requires subquery, but we try to optimize main path.
    v_sort_logic := (CASE 
        ...
        ELSE 'p.created_at' -- Default fallback
    ...
    -- We select IDs first using pure index scan (ideally), then Join.
    ...
    -- Re-sort to ensure order matches CTE (as JOIN order is undefined)
    ORDER BY ' || v_sort_logic;
