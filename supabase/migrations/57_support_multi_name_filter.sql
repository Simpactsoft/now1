-- Migration 57: Support Multiple Name Filters (AND Logic)
-- Purpose: Allow filtering by multiple name conditions (e.g., Contains "A" AND Contains "B").
-- Changes: Update fetch_people_crm to handle jsonb array for 'name' filter.
...
BEGIN
    ...
    -- [NAME COLUMN FILTER - UPDATED TO SUPPORT ARRAY]
    IF arg_filters->'name' IS NOT NULL THEN
        IF jsonb_typeof(arg_filters->'name') = 'array' THEN
            ...
            END LOOP;
        ELSE
            -- Single value (Old Logic)
            v_name_filter := arg_filters->>'name';
            IF length(v_name_filter) > 0 THEN
