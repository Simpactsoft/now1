...
-- Migration: 132_nuclear_performance.sql
-- Description: The "Nuclear" Performance Fix.
-- 1. Enables 'btree_gist' extension to allow combining UUID (Tenant) and LTREE (Hierarchy) in a single GIST index.
-- 2. Creates the "Holy Grail" Composite Index (tenant_id, hierarchy_path).
-- 3. Simplifies fetch_people_crm to REMOVE the 'OR agent_id' fallback, forcing strict GIST usage.

BEGIN;

-- 1. Enable btree_gist (Standard Contrib module)
CREATE EXTENSION IF NOT EXISTS btree_gist;

-- 2. Create the Composite Index
-- This allows Postgres to filter by Tenant AND Hierarchy in a single index hop.
DROP INDEX IF EXISTS idx_cards_tenant_hierarchy_composite;
CREATE INDEX idx_cards_tenant_hierarchy_composite 
ON cards USING GIST (tenant_id, hierarchy_path);

-- 3. Ensure Backfill is complete (Just in case)
UPDATE cards c
...
-- 4. Rewrite fetch_people_crm to use the Composite Index logic strictly
...
SET statement_timeout = '20000ms' -- 20s Timeout
AS $$
DECLARE
    v_total_rows bigint;
    v_query text;
...
BEGIN
    ...
    -- [Security Injection]
    -- STRICT GIST OPTIMIZATION: No OR conditions with non-indexed columns.
    IF NOT v_is_distributor THEN
        IF v_auth_org_path IS NOT NULL THEN
             -- Only use the Composite GIST logic
             -- The index (tenant_id, hierarchy_path) matches: tenant_id = X AND hierarchy_path <@ Y
             v_where_clause := v_where_clause || format(' AND c.hierarchy_path <@ %L::ltree ', v_auth_org_path::text);
        ELSE
             -- Fallback if user has no path (shouldn't happen for valid users, but safety)
    ...
    -- [Main Query]
    -- Using the CTE trick again, but now backed by the Composite Index
    v_query := '
    WITH visible_ids AS (
        SELECT c.id
        FROM cards c
