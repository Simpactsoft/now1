-- Migration: Fix People Status Filter Logic
-- Description: Updates get_people_count and fetch_people_data to coalesce `p.status` to 'lead' within filter conditions, preventing zero records when filtering by status for people without an explicit status value.
...
BEGIN
    ...
    -- Search (Improved to check multiple fields AND normalize spaces)
    IF arg_filters->>'search' IS NOT NULL AND length(arg_filters->>'search') > 0 THEN
         v_where_clause := v_where_clause || ' AND (
            regexp_replace(p.display_name, ''\s+'', '' '', ''g'') ILIKE ''%'' || regexp_replace(' || quote_literal(arg_filters->>'search') || ', ''\s+'', '' '', ''g'') || ''%''
            OR regexp_replace(coalesce(p.first_name, ''''), ''\s+'', '' '', ''g'') ILIKE ''%'' || regexp_replace(' || quote_literal(arg_filters->>'search') || ', ''\s+'', '' '', ''g'') || ''%''
            OR regexp_replace(coalesce(p.last_name, ''''), ''\s+'', '' '', ''g'') ILIKE ''%'' || regexp_replace(' || quote_literal(arg_filters->>'search') || ', ''\s+'', '' '', ''g'') || ''%''
            OR regexp_replace(coalesce(p.first_name, '''') || '' '' || coalesce(p.last_name, ''''), ''\s+'', '' '', ''g'') ILIKE ''%'' || regexp_replace(' || quote_literal(arg_filters->>'search') || ', ''\s+'', '' '', ''g'') || ''%''
            OR coalesce(p.email, '''') ILIKE ''%'' || ' || quote_literal(arg_filters->>'search') || ' || ''%''
         ) ';
    END IF;

    -- Status (Fixed COALESCE for NULL status values handling)
    IF arg_filters->'status' IS NOT NULL AND jsonb_typeof(arg_filters->'status') = 'array' THEN
        v_where_clause := v_where_clause || format(' AND lower(coalesce(p.status, ''lead'')) IN (SELECT lower(x) FROM jsonb_array_elements_text(%L::jsonb) t(x)) ', arg_filters->'status');
    ELSIF arg_filters->>'status' IS NOT NULL THEN
        v_where_clause := v_where_clause || format(' AND lower(coalesce(p.status, ''lead'')) = lower(%L) ', arg_filters->>'status');
    END IF;
