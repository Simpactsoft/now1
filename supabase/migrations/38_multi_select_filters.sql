-- Migration: Support Multi-Select Filters
-- Updates fetch_people_crm to accept arrays for status, role, year, etc.
...
BEGIN
    ...
    -- Status (Multi-Select or Single)
    IF arg_filters->'status' IS NOT NULL THEN
        IF jsonb_typeof(arg_filters->'status') = 'array' THEN
            -- Check if any status in array matches (Case Insensitive not easy with simple ANY array, so we unnest)
    ...
    -- Company Size (Multi-Select or Single)
    -- Keeping Single for now or Array? Implementation Plan said Status/Role/Year. Let's do just those to be safe first.
    -- But logical consistency is good. Let's stick to Status/Role/Year first as requested.
    
    -- Joined Year (Multi-Select)
    ...
    -- Tags (Multi-Select) - Assuming OR logic for tags? Or AND? 
    -- Usually filter A OR B.
