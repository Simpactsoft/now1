-- Migration 55: Increase RPC Timeout
-- Purpose: Prevent "canceling statement due to statement timeout" errors during complex Search + Filter operations.
-- Context: Combining GIN Index (Search) with B-Tree (Status/Role) can sometimes be computationally expensive if the planner misestimates.
-- Solution: Increase statement_timeout for this specific transaction to 5s (5000ms).
...
DECLARE
    ...
    v_tags_filter text[];
BEGIN
    -- 0. Parse JSON Filters (Same Logic)
