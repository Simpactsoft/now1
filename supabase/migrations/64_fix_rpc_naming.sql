-- Migration 64: Fix RPC Naming (Bypass Cache)
-- Rename functions to force fresh registry detection

-- 1. SUBMIT Search History (was add_search_history)
...
-- 2. LIST Search History (was get_search_history)
...
-- 3. WIPE Search History (was clear_search_history)
...
-- Drop old ones to clean up (optional, but good practice)
drop function if exists add_search_history(uuid, text);
drop function if exists get_search_history(uuid);
drop function if exists clear_search_history(uuid);
