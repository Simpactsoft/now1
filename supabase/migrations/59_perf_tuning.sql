-- Migration 59: Performance Tuning for Search
-- 1. Ensure Trigram Extension
create extension if not exists pg_trgm;

-- 2. Add GIN Index on display_name for fast ILIKE/Regex search
create index if not exists idx_parties_display_name_gin 
  on parties using gin (display_name gin_trgm_ops);

-- 3. Add GIN Index on tags if not exists (already checked earlier, but ensuring safety)
create index if not exists idx_parties_tags_gin 
  on parties using gin (tags);

-- 4. Update RPC to have higher timeout (20s) and ensuring use of indexes
...
SET statement_timeout = 30000 -- Increase to 30s
AS $$
DECLARE
    v_total_rows bigint;
    v_query text;
    ...
    v_tags_filter text[];
BEGIN
    -- 0. Parse JSON Filters code (same as before) --
    ...
    IF arg_filters->>'search' IS NOT NULL AND length(arg_filters->>'search') > 0 THEN
         ...
         IF array_length(v_search_parts, 1) = 1 THEN
             -- Use ILIKE with % for trigram support
    ...
    IF arg_filters->'name' IS NOT NULL THEN
        IF jsonb_typeof(arg_filters->'name') = 'array' THEN
            FOR v_name_val IN SELECT * FROM jsonb_array_elements_text(arg_filters->'name')
            LOOP
                IF length(v_name_val) > 0 THEN
                    IF length(v_name_val) < 3 THEN
                         v_where_clause := v_where_clause || ' AND lower(p.display_name) LIKE ' || quote_literal(lower(v_name_val) || '%');
                    ELSE
                         v_where_clause := v_where_clause || ' AND p.display_name ILIKE ''%'' || ' || quote_literal(v_name_val) || ' || ''%'' ';
                    END IF;
                END IF;
            END LOOP;
        ELSE
            -- Single value
            IF length(arg_filters->>'name') > 0 THEN
                IF length(arg_filters->>'name') < 3 THEN
                    v_where_clause := v_where_clause || ' AND lower(p.display_name) LIKE ' || quote_literal(lower(arg_filters->>'name') || '%');
                ELSE
    ...
    -- 3. Get Total Count (ONLY if needed? No, always needed for pagination UI)
    -- Optimize count: If using ILIKE with trigram, COUNT(*) should be faster.
