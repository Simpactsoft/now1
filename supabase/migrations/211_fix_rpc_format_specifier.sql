...
-- Migration: 211_fix_rpc_format_specifier.sql
-- Description: Fixes "unrecognized format() type specifier" error by properly escaping % as %% inside format() strings.

BEGIN;

-- 1. Update Count Function (Fix Escaping)
...
BEGIN
    ...
    -- Search (Not using format(), so single % is fine)
    ...
    -- Role (Fix: Use ''%%'' inside format() to output literal %)
...
-- 2. Update Data Function (Fix Escaping in Role Logic)
