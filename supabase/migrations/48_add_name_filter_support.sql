-- Migration 48: Add Name Filter Support
-- Fixes: "Column Filter does not work" when combined with Search.
-- Adds: Specific logic for 'name' filter, using the same Hybrid Strategy (Prefix/Trigram).
...
DECLARE
    ...
    -- Filter Variables
    v_search_term text;
    v_name_filter text; -- [NEW]
    v_status_filter text[];
    v_role_filter text[];
    v_year_filter text[];
    v_tags_filter text[];
BEGIN
    -- 0. Parse JSON Filters into Native Arrays/Vars
    ...
    -- [NAME COLUMN FILTER] (New Support)
    -- Works in parallel with Global Search (AND logic)
