-- Migration: Fix Missing Role in Return
-- Updates fetch_people_crm to return ret_role_name

-- Drop first because return signature changed
...
RETURNS TABLE (
    ...
    ret_status text, 
    ret_rating int,
    ret_role_name text,  -- [NEW] Added Role
...
BEGIN
    ...
    v_query := 'SELECT 
        ...
        coalesce(p.rating, 0),
        -- [NEW] Fetch Primary Role (First one found)
