-- Phase 14 Fix: Correct Aliases in Fetch Employees RPC
-- The previous version caused "missing FROM-clause entry for table p" because the CTE used p_inner/m_inner while the sort clause used p/m.
...
BEGIN
    ...
    -- We map the sort columns to the new structure (using aliases p and m)
    v_order_clause := (CASE
    ...
    -- FIX: We consistently use aliases 'm' and 'p' in the CTE so v_order_clause (which uses p/m) works correctly.
    v_dynamic_query := 'WITH keys AS (
        SELECT m.id FROM party_memberships m
        JOIN parties p ON m.person_id = p.id
        WHERE m.tenant_id = $1 AND ($2 = '''' OR p.display_name ILIKE ''%'' || $2 || ''%'')
