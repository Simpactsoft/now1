...
-- Migration: 130_optimize_data_rpc.sql
-- Description: Converts fetch_people_crm to SECURITY DEFINER with Manual Security Injection.
-- Solves the "Statement Timeout" caused by RLS+Sort index scans on large datasets.
...
SET statement_timeout = '10000ms' -- Grant 10s for this query
AS $$
DECLARE
    v_total_rows bigint;
    v_query text;
...
BEGIN
    ...
    IF v_auth_tenant_id IS DISTINCT FROM arg_tenant_id THEN
        -- Return Empty if tenant mismatch
        -- We return 0 rows
        RETURN; 
    END IF;

    -- 2. Construct WHERE Clause (Base)
    -- Using alias 'c' for cards
    v_where_clause := format(' WHERE c.tenant_id = %L AND c.type = ''person'' ', arg_tenant_id);

    -- 3. Security Injection: Hierarchy
    IF NOT v_is_distributor THEN
        -- Manual implementation of the "Hierarchy Visibility" Logic
        -- We prioritize the Index-Friendly <@ operator
    ...
    -- [Industry] (Adding missing filter from 126)
    ...
    -- 5. Count Calculation
    -- We run this inside the Security Definer context too
