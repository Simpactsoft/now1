...
-- Migration: 136_restore_performance_indexes.sql
-- Description:
-- 1. Restores 'idx_cards_tenant_id' (dropped in 134) which is critical for the '135' simple queries.
-- 2. Increases statement_timeout for 'fetch_people_crm' to 60s to allow counting 1.3M rows.
-- 3. Ensures 'created_at' index exists for sorting.

BEGIN;

-- 1. Restore the fundamental Index (Dropped in 134)
CREATE INDEX IF NOT EXISTS idx_cards_tenant_id ON cards(tenant_id);

-- 2. Ensure Sorting Index exists
CREATE INDEX IF NOT EXISTS idx_cards_created_at_desc ON cards(created_at DESC);

-- 3. Update Function with Timeout Boost (60s)
...
SET statement_timeout = '60000ms' -- Grant 60s for counting 1.3M rows
AS $$
DECLARE
    v_total_rows bigint;
    v_query text;
...
BEGIN
    ...
    -- [Count]
    -- Optimized Count using the specific Index if possible
